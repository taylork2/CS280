...
{
    cerr << "Type error performing " << type << " operator at line " << line+1 << endl;
	exit(0);
}
...
PTreeUnion::eval()
{
	...
	list<SearchTool::SearchResultEntry *> result;
	result = sea->getResults();

	//create new SearchResult for the value 
	SearchTool::SearchResult *sea2;
	sea2 = new SearchTool::SearchResult(file, result);

	//do second search, push_back these new SearchResultEntrys to results
	if (!(sea2 -> dosearch(searchfor))){
		TypeError("UNION", line);
	}
	Value *returnVal;
	returnVal = new ValueSearchResult(sea2);
    return returnVal;	
}
...
{	
	try { 
		...
		cout << "Variable " << id << " not set before use at line " << line+1 << endl;
	}  
}
